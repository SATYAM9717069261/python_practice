import pprint # only show Better Output (View)
...
def inheritance(instance):
    if hasattr(instance.__class__,'__mro__'):   # hasattr check that attribute in class Or Not '__mro__' is Hidden Attribute of '__class__'
        return (instance , ) +instance .__class__.__mro__     # mro gives (order of) Class Inhertance 
    else:
        return [instance] + dfir(istance,__class__)
...
if __name__=='__main__':
    print('Classiic classes in 2.x , new-style in 3.x')
    class A: attr1=1
    class B(A): attr2=2
    class C(A): attr1=3
    class D(B,C): pass
